...
void Person::setPhoneNumber(string phon) { phoneNumber = phon; }
...
//===========================================
// 2. Patient Class Definitions
//===========================================
Patient::Patient() : Person(), patientId(" "), bloodGroup(" "), dateOfAdmission(" "), username(" "), password(" ") {
	historyCount = 0;
}
...
void Patient::setBloodGroup(string bGroup) { bloodGroup = bGroup; }
void Patient::setDateOfAdmission(string doa) { dateOfAdmission = doa; }
void Patient::setUsername(string user) { username = user; }
void Patient::setpassword(string pass) { password = pass; }

string Patient::getPatientId() const { return patientId; }
string Patient::getBloodGroup() const { return bloodGroup; }
string Patient::getDateOfAdmission() const { return dateOfAdmission; }
int Patient::getHistoryCount() const { return historyCount; }
string Patient::getUsername() const { return username; }
string Patient::getPassword() const { return password; }
...
string Patient::getHistoryItem(int index) const {
...
bool Patient::loadFromFile(ifstream& inFile) {
...
void Employee::setEmployeeId(string eid) { employeeId = eid; }
void Employee::setSalary(double sa) { salary = sa; }
void Employee::setDateOfJoining(string doj) { dateOfJoining = doj; }

string Employee::getEmployeeId() const { return employeeId; }
double Employee::getSalary() const { return salary; }
string Employee::getDateOfJoining() const { return dateOfJoining; }
...
//===========================================
// 4. Doctor Class Definitions
//===========================================
Doctor::Doctor() : Employee(), specialization(" "), licenseNumber(" "), scheduale(" "), username(" "), password(" ") {}
...
void Doctor::setLicenseNumber(string ln) { licenseNumber = ln; }
void Doctor::setScheduale(string sched) { scheduale = sched; }
void Doctor::setUserName(string user) { username = user; }
void Doctor::setPassword(string pass) { password = pass; }

string Doctor::getSpecialization() const { return specialization; }
string Doctor::getLicenseNumber() const { return licenseNumber; }
string Doctor::getSchedual() const { return scheduale; }
string Doctor::getUserName() const { return username; }
string Doctor::getPassword() const { return password; }
...
bool Doctor::loadFile(ifstream& ifile) {
...
//===========================================
// 5. AdminStaff Class Definitions
//===========================================
AdminStaff::AdminStaff() {
	departmentName = " ";
}

AdminStaff::AdminStaff(string d, string e, double s, string doj, string n, int a, string addr, string p)
	: Employee(n, a, addr, p, e, s, doj), departmentName(d) {
}

void AdminStaff::setDepName(string depName) { departmentName = depName; }
string AdminStaff::getDepName() const { return departmentName; }

void AdminStaff::manageStaff() {
	cout << "Admin Staff: Managing hospital personnel for " << departmentName << ".\n";
}

void AdminStaff::handleBilling() {
	cout << "Admin Staff: Processing medical invoices and billing.\n";
}

double AdminStaff::calculateSalary() {
	return getSalary() * 1.10;
}

void AdminStaff::saveToFile() {
...
bool AdminStaff::loadFromFile(ifstream& ifile) {
...
//===========================================
// 6. Appointment Class Definitions
//===========================================
...
Appointment::Appointment(string a, string d, string t, string s, Patient p, Doctor doc)
	: appointmentID(a), date(d), time(t), status(s), patient(p), doctor(doc) {
}

void Appointment::setAppointmentID(string a) { appointmentID = a; }
...
bool Appointment::loadFile(ifstream& ifile) {
...
Room::Room(string n, string t, bool i)
	: roomNumber(n), roomType(t), isOccupied(i) {
}

void Room::setRoomNumber(string n) { roomNumber = n; }
void Room::setRoomType(string t) { roomType = t; }
void Room::setIsOccupied(bool i) { isOccupied = i; }
...
void Room::assignPatient(string patientName) {
	if (!isOccupied) {
		isOccupied = true;
...
bool Room::loadFile(ifstream& ifile) {
	string rNum, rType;
	bool status;
...
//===========================================
// 8. MedicalRecord Class Definitions
//===========================================
...
MedicalRecord::MedicalRecord(string r, string d, string p, string dat, string pName, string pID)
	: recordID(r), diagnosis(d), prescription(p), date(dat), patientName(pName), patientID(pID) {
}

void MedicalRecord::setRecordID(string r) { recordID = r; }
void MedicalRecord::setDiagnosis(string d) { diagnosis = d; }
void MedicalRecord::setPrescription(string p) { prescription = p; }
void MedicalRecord::setDate(string d) { date = d; }
void MedicalRecord::setPatientName(string pName) { patientName = pName; }
void MedicalRecord::setPatientId(string pID) { patientID = pID; }
...
void MedicalRecord::addEntry(string id, string diag, string rx, string d, Patient& p) {
	recordID = id;
	diagnosis = diag;
	prescription = rx;
	date = d;
...
void MedicalRecord::getRecordByDate(string d) {
	if (date == d)
		display();
	else
...
bool MedicalRecord::loadFile(ifstream& ifile) {
...
//===========================================
// 9. Bill Class Definitions
//===========================================
Bill::Bill() : patient(), room() {
	billId = " ";
	amount = 0;
...
Bill::Bill(string bi, Patient p, Room r) : patient(p), room(r) {
	billId = bi;
	itemCount = 0;
	amount = 0;
	paymentStatus = "Pending";
...
void Bill::AddItems(string item) {
	if (itemCount < 50) {
...
void Bill::setBillId(string bi) { billId = bi; }
...
string Bill::getPatientID() const { return patient.getPatientId(); }

void Bill::genrateBill() {
...
bool Bill::loadFile(ifstream& ifile) {
...
void Hospital::addEmployee(AdminStaff* a) {
	if (eCount < 100) {
		staff[eCount] = a;
		eCount++;
		a->saveToFile();
...
void Hospital::bookNewAppointment() {
...
bool Hospital::isPatientIdValid(string id) {
	ifstream ifile(patientFile);
	Patient p;
	if (!ifile.is_open()) {
		return false;
...
void Hospital::updatePatient() {
	...
	while (p.loadFromFile(ifile)) {
		...
		ofile << p.getUsername() << endl;
		ofile << p.getPassword() << endl;
...
void Hospital::viewMedicalRecords() {
...
void Hospital::viewMyDetails(string myID) {
	ifstream ifile(patientFile);
	if (!ifile.is_open()) {
...
void Hospital::viewMyHistory(string myID) {
	ifstream ifile(patientFile);
	if (!ifile.is_open()) {
		cout << "Error: Records file missing." << endl;
		return;
...
void Hospital::viewMyAppointments(string myName) {
	ifstream ifile(apptFile);
	if (!ifile.is_open()) {
...
void Hospital::viewMyBills(string myID) {
...
//===========================================
// 11. Authentication Class Definitions
//===========================================
...
void Authentication::setUserRoll(string role) { userrole = role; }
...
void Authentication::registerUser(string user, string pass, string role) {
	username = user;
	password = pass;
	userrole = role;
	...
	do {
		...
		case 2: {
			...
			if (hospital.isPatientIdValid(patientId)) {
				...
				cout << "Enter Bill Id:";
				getline(cin, billId);
				Patient pt(patientName, 0, " ", " ", patientId, " ", " ", " ", " ");
				Room r(" ", rType, true);
